...
public:
    ...
    void RenderSceneCB() override
    {
        ...
        submitInfo.pCommandBuffers      = m_swapchain->CommandBuffer(ImageIndex);
    ...
    void RecordCommandBuffers() {
        ...
        for (uint i = 0 ; i < m_swapchain->ImageCount() ; i++) {
            auto commandBuffer = *(m_swapchain->CommandBuffer(i));

            vkBeginCommandBuffer(commandBuffer, &beginInfo);

                vkCmdClearColorImage(commandBuffer, m_swapchain->Image(i), VK_IMAGE_LAYOUT_GENERAL, &clearColor, 1, &imageRange);
